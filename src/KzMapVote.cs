...
static class MapResolver
{
    ...
    public static async Task<(List<MapEntry> maps, string? err)> ApiGetMaps(HttpClient http, string? name = null, string? state = "approved", int? limit = null, int? offset = null)
    {
        try {
            ...
            return (new List<MapEntry>(), $"Failed to fetch maps from CS2KZ API: {e.Message}");
        }
    }
...
public partial class KzMapVote : BasePlugin {
    ...
    public override void Load(bool hotReload) {
        ...
        Core.Event.OnMapLoad += (@event) => {
            ...
            ResetMapChangeState();
            Task.Run(async () => {
                var (maps, err) = await MapResolver.ApiGetMaps(m_http_client);
                if (err is not null) {
                    Core.Logger.LogError($"Failed to refresh map pool: {err}");
                    return;
                }
    ...
    public void Nominate(ICommandContext ctx) {
        ...
        Task.Run(async () => {
            var (entry, err) = await MapResolver.Resolve(m_http_client, m_config.SteamApiKey, input);
            if (err is not null || entry is null)
            {
                Core.Logger.LogWarning($"Failed to nominate '{input}': {err}");
                _ = ctx.Sender.SendChatAsync($"Failure: {err}");
                return;
            }
